...
void ProgramImplCore::ordered_reset(SequenceState& sequence) {
    ...
    if (peer) {
        ...
        // `io.token` carries the last sampled token, and the MTP prompt alignment reads it
        // as the token generated at the final column. A fresh request has sampled nothing, so
        // leaving the previous request's token here seeds this one with the last request.
        set_peer_i32(peer->io.token, 0);
        if (peer->io.mtp) {
            set_peer_i32(peer->io.mtp->position, 0);
            // The draft window is read back as proposals whenever the request's draft
            // count is non-zero, which it is on every MTP request. Left over, a previous
            // request's draft is eligible to be accepted and emitted as this one's answer.
            CUDA_CHECK(cudaMemsetAsync(peer->io.mtp->draft_tokens.data, 0,
                                       peer->io.mtp->draft_tokens.bytes(),
                                       peer->device.stream));
        }
    }
    work.reset();
    ...
    set_device_i32(io.token, 0);
    if (io.mtp) {
        set_device_i32(io.mtp->position, 0);
        CUDA_CHECK(cudaMemsetAsync(io.mtp->draft_tokens.data, 0,
                                   io.mtp->draft_tokens.bytes(), device.stream));
    }
    sequence.text_kv_valid           = 0;
    sequence.mtp_kv_valid            = 0;
    sequence.dflash_context_frontier = 0;
